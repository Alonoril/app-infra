...
crate::define_pub_schema!(TtlSingleSchema, TtlSingleKey, TtlSingleValue, "ttl_single_index");
...
impl RksDB {
	...
	pub fn put_with_ttl<S: Schema>(&self, key: &S::Key, value: &S::Value, expire_at: u64) -> AppResult<()> {
		// Validate expiration time
		let current_time = current_timestamp();
		if expire_at <= current_time {
			return Err(crate::errors::invalid_ttl_expire_at());
		}
	...
	pub fn cleanup_expired_with_batch_size(&self, current_time: u64, max_batch_size: usize) -> AppResult<()> {
		if max_batch_size == 0 {
			return Err(crate::errors::invalid_batch_size());
		}
	...
	fn batch_delete_expired(&self, expired_keys: &[(TtlExpirationKey, TtlExpirationValue)]) -> AppResult<()> {
		if expired_keys.is_empty() {
			return Ok(());
		}
...
mod tests {
	use super::*;
	use crate::typed_db::schema::Schema;
	use serde::{Deserialize, Serialize};
	...
	fn put_expired_for_test(db: &RksDB, key: &TestKey, value: &TestValue, expire_at: u64) {
		let batch = SchemaBatch::new();
		batch.put::<TestSchema>(key, value).unwrap();

		let original_key = bincode::encode_to_vec(key, bincode::config::standard()).unwrap();
		let schema_name = std::any::type_name::<TestSchema>().to_string();
		let ttl_expiration_key = TtlExpirationKey {
			expire_timestamp: expire_at,
			schema_name: schema_name.clone(),
		...
		let ttl_expiration_value = TtlExpirationValue {
			cf_name: TestSchema::COLUMN_FAMILY_NAME.to_string(),
		};
		let ttl_single_key = TtlSingleKey {
			schema_name,
			original_key,
		...
		batch
			.put::<TtlExpirationSchema>(&ttl_expiration_key, &ttl_expiration_value)
			.unwrap();
		batch
			.put::<TtlSingleSchema>(&ttl_single_key, &ttl_single_value)
			.unwrap();
	...
	fn test_expired_data_removal() {
		...
		put_expired_for_test(&db, &key, &value, past_time);
	...
	fn test_cleanup_expired() {
		...
		let ttl_single_key = TtlSingleKey {
			...
			original_key: bincode::encode_to_vec(&key, bincode::config::standard()).unwrap(),
		};

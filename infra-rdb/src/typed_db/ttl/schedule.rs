use crate::typed_db::RksDB;
use infra_core::{result::AppResult, rt_pool::Tokio};
use std::{
	sync::{
		Arc,
...
impl RksdbTtlScheduler {
	...
	pub async fn stop(&mut self) -> AppResult<()> {
		...
		if let Some(shutdown_tx) = self.shutdown_tx.take()
			&& let Err(e) = shutdown_tx.send(()).await
		{
...
impl RksdbTtlSchedulerManager {
	...
	pub fn all_running(&self) -> bool {
		!self.schedulers.is_empty() && self.schedulers.iter().all(|scheduler| scheduler.is_running())
	}
}
...
mod tests {
	use super::*;
	use crate::typed_db::{Schema, SchemaBatch};
	...
	fn put_expired_for_test(db: &RksDB, key: &TestKey, value: &TestValue, expire_at: u64) {
		...
		let ttl_expiration_key = super::super::TtlExpirationKey {
			expire_timestamp: expire_at,
			schema_name: schema_name.clone(),
			original_key: original_key.clone(),
		};
		let ttl_expiration_value = super::super::TtlExpirationValue {
			cf_name: TestSchema::COLUMN_FAMILY_NAME.to_string(),
		};
		let ttl_single_key = super::super::TtlSingleKey {
			schema_name,
			original_key,
		};
		let ttl_single_value = super::super::TtlSingleValue {
			expire_timestamp: expire_at,
			cf_name: TestSchema::COLUMN_FAMILY_NAME.to_string(),
		};

		batch
			.put::<super::super::TtlExpirationSchema>(&ttl_expiration_key, &ttl_expiration_value)
			.unwrap();
		batch
			.put::<super::super::TtlSingleSchema>(&ttl_single_key, &ttl_single_value)
			.unwrap();

use crate::typed_db::ColumnFamilyName;
...
macro_rules! define_schema {
	($schema_type:ident, $key_type:ty, $value_type:ty, $cf_name:expr) => {
		...
		impl $crate::typed_db::schema::Schema for $schema_type {
			type Key = $key_type;
			type Value = $value_type;

			const COLUMN_FAMILY_NAME: $crate::typed_db::ColumnFamilyName = $cf_name;
		}
	};
}
...
/// This trait defines a typed_db: an association of a column family name, the key type and the value
/// type.
pub trait Schema: Debug + Send + Sync + 'static {
	/// The column family name associated with this struct.
	/// Note: all wallets within the same TypedDB must have distinct column family names.
...
#[cfg(feature = "fuzzing")]
pub mod fuzzing {
	use crate::typed_db::{KeyCodec, Schema, ValueCodec};
	...
	/// Helper used in tests and fuzzers to make sure a typed_db never panics when decoding random bytes.
	...
	macro_rules! test_no_panic_decoding {
		($schema_type:ty) => {
			use infra_rdb::typed_db::schema::fuzzing::{arb_small_vec_u8, assert_no_panic_decoding};

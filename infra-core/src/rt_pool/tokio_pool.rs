use crate::rt_pool::MAX_THREAD_NAME_LENGTH;
...
static APP_RT: std::sync::LazyLock<Runtime> =
	std::sync::LazyLock::new(|| build_named_runtime("tokio-rt", Some(num_cpus::get() * 2)));
	...
	F: Fn() + Send + Sync + 'static,
{
	...
	if thread_name.len() > MAX_THREAD_NAME_LENGTH {
		panic!(
			"The given runtime thread name is too long! Max length: {MAX_THREAD_NAME_LENGTH}, given name: {thread_name}",
		);
	}
	...
	builder
		.build()
		.unwrap_or_else(|error| panic!("Failed to spawn named runtime! Name: {thread_name:?}, Error: {error:?}",))
}

// pub fn spawn_main_thread<F>(&self, future: F) -> std::thread::JoinHandle<()>
// where
...
// fn spawn_main_thread<F>(fut: F) -> std::thread::JoinHandle<()>
// where
// 	F: Future<Output = ()> + Send + 'static,
// {
// 	std::thread::spawn(|| {

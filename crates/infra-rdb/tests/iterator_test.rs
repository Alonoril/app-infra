...
use infra_core::result::{AppResult, ErrCodeTrait};
use infra_rdb::{
	define_schema,
	errors::RdbErr,
	typed_db::{
		IntoDbResult, RksDB, SchemaIterator,
...
impl KeyCodec<TestSchema> for TestKey {
	fn encode_key(&self) -> AppResult<Vec<u8>> {
		let mut bytes = vec![];
		bytes.write_u32::<BigEndian>(self.0).into_db_res()?;
		bytes.write_u32::<BigEndian>(self.1).into_db_res()?;
		bytes.write_u32::<BigEndian>(self.2).into_db_res()?;
		Ok(bytes)
	}
	...
	fn decode_key(data: &[u8]) -> AppResult<Self> {
		...
		Ok(TestKey(
			reader.read_u32::<BigEndian>().into_db_res()?,
			reader.read_u32::<BigEndian>().into_db_res()?,
			reader.read_u32::<BigEndian>().into_db_res()?,
		))
	}
...
impl ValueCodec<TestSchema> for TestValue {
	...
	fn decode_value(data: &[u8]) -> AppResult<Self> {
		let mut reader = std::io::Cursor::new(data);
		Ok(TestValue(reader.read_u32::<BigEndian>().into_db_res()?))
	}
}
...
fn collect_incomplete(iter: &mut SchemaIterator<TestSchema>) -> Vec<u32> {
	...
	for res in iter {
		match res {
			...
			Err(e) => {
				if e.domain() == RdbErr::RocksDbIncompleteResult.domain()
					&& e.code() == RdbErr::RocksDbIncompleteResult.code()
				{
					return res_vec;
				}
...
impl TestDB {
	fn new() -> Self {
		...
		let db = RksDB::open(tmpdir.path(), "test", column_families, &db_opts).unwrap();

		db.put::<TestSchema>(&TestKey(1, 0, 0), &TestValue(100)).unwrap();
		db.put::<TestSchema>(&TestKey(1, 0, 2), &TestValue(102)).unwrap();
		db.put::<TestSchema>(&TestKey(1, 0, 4), &TestValue(104)).unwrap();
		db.put::<TestSchema>(&TestKey(1, 1, 0), &TestValue(110)).unwrap();
		db.put::<TestSchema>(&TestKey(1, 1, 2), &TestValue(112)).unwrap();
		db.put::<TestSchema>(&TestKey(1, 1, 4), &TestValue(114)).unwrap();
		db.put::<TestSchema>(&TestKey(2, 0, 0), &TestValue(200)).unwrap();
		db.put::<TestSchema>(&TestKey(2, 0, 2), &TestValue(202)).unwrap();
...
fn test_seek_to_first() {
	...
	iter.seek_to_first();
	assert_eq!(collect_values(iter), [100, 102, 104, 110, 112, 114, 200, 202]);
...
fn test_seek_to_last() {
	...
	let mut iter = db.rev_iter();
	iter.seek_to_last();
	assert_eq!(collect_values(iter), [202, 200, 114, 112, 110, 104, 102, 100]);
}
...
impl TestDBWithPrefixExtractor {
	fn new() -> Self {
		...
		// delete later
		db.put::<TestSchema>(&TestKey(1, 1, 1), &TestValue(111)).unwrap();
		db.put::<TestSchema>(&TestKey(1, 2, 2), &TestValue(122)).unwrap();
		db.put::<TestSchema>(&TestKey(1, 2, 3), &TestValue(123)).unwrap();

		//  delete without update TestKey(1,2,4)

		db.put::<TestSchema>(&TestKey(1, 2, 5), &TestValue(125)).unwrap();
		db.put::<TestSchema>(&TestKey(1, 5, 5), &TestValue(155)).unwrap();
		// delete later
		db.put::<TestSchema>(&TestKey(1, 6, 6), &TestValue(166)).unwrap();
		// to be overwritten
		db.put::<TestSchema>(&TestKey(1, 7, 7), &TestValue(0)).unwrap();
		// overwrite
		db.put::<TestSchema>(&TestKey(1, 7, 7), &TestValue(177)).unwrap();
		db.put::<TestSchema>(&TestKey(2, 2, 2), &TestValue(222)).unwrap();
		// delete later
		db.put::<TestSchema>(&TestKey(2, 3, 3), &TestValue(233)).unwrap();

		// delete without update TestKey(2,4,4)

		db.put::<TestSchema>(&TestKey(2, 7, 7), &TestValue(277)).unwrap();
		db.put::<TestSchema>(&TestKey(2, 8, 8), &TestValue(288)).unwrap();
		db.put::<TestSchema>(&TestKey(2, 9, 9), &TestValue(299)).unwrap();
		// delete later
		db.put::<TestSchema>(&TestKey(3, 7, 7), &TestValue(377)).unwrap();
		// delete later
		db.put::<TestSchema>(&TestKey(3, 8, 8), &TestValue(388)).unwrap();
		db.put::<TestSchema>(&TestKey(3, 9, 9), &TestValue(399)).unwrap();
		...
		TestDBWithPrefixExtractor { _tmpdir: tmpdir, db }
	}
	...
	fn iter_with_same_prefix(&self) -> SchemaIterator<TestSchema> {
		...
		self.db.iter_with_opts(opts).expect("Failed to create iterator.")
	}
...
fn test_iter_with_max_skipped_deletions() {
	...
	iter.seek(&TestKey(1, 5, 5)).unwrap();
	assert_eq!(collect_incomplete(&mut iter), [155, 177, 222, 277, 288, 299]);
...
fn test_iter_with_upper_bound() {
	...
	let mut iter = db.iter_with_upper_bound(KeyCodec::<TestSchema>::encode_key(&TestKey(1, 5, 5)).unwrap());
	iter.seek_to_first();
	assert_eq!(collect_values_mut(&mut iter), [122, 123, 125]);

	let mut iter = db.iter_with_upper_bound(KeyCodec::<TestSchema>::encode_key(&TestKey(1, 2, 4)).unwrap());
	iter.seek_to_first();
	assert_eq!(collect_values_mut(&mut iter), [122, 123]);

	let mut iter = db.iter_with_upper_bound(SeekKeyCodec::<TestSchema>::encode_seek_key(&KeyPrefix1(2)).unwrap());
	iter.seek(&KeyPrefix1(1)).unwrap();
	assert_eq!(collect_values_mut(&mut iter), [122, 123, 125, 155, 177]);

	let mut iter = db.iter_with_upper_bound(SeekKeyCodec::<TestSchema>::encode_seek_key(&KeyPrefix2(1, 5)).unwrap());
	iter.seek(&KeyPrefix1(1)).unwrap();
	assert_eq!(collect_values_mut(&mut iter), [122, 123, 125]);
}

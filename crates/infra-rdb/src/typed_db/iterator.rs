	...
	S: Schema,
{
	...
	/// `seek_key`.
	pub fn seek<SK>(&mut self, seek_key: &SK) -> AppResult<()>
	where
		SK: SeekKeyCodec<S>,
	{
	...
	pub(crate) fn seek_bytes(&mut self, seek_key: &[u8]) {
		self.db_iter.seek(seek_key);
		self.status = Status::DoneSeek;
	}

	pub(crate) fn current_key_starts_with(&self, prefix: &[u8]) -> bool {
		self.db_iter.key().is_some_and(|key| key.starts_with(prefix))
	}
	...
	pub fn seek_for_prev<SK>(&mut self, seek_key: &SK) -> AppResult<()>
	where
		SK: SeekKeyCodec<S>,
	{
		let key = <SK as SeekKeyCodec<S>>::encode_seek_key(seek_key)?;
